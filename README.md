# DDA2001 Introduction to Data Science-25 Spring
> CUHK(SZ) · School of Data Science · Academic Year 25-26 Spring
>
> Instructor: Shuang Li

---
   
## Course Description 
This repository contains the complete study notes of the course DDA2001 Introduction to Data Science, including the notes, homeworks, slides of the total 25 lectures. The notes are primarily based on the slides, supplemented with the textbook derivations, intuitive explanitions, and high-confidence academic references.

---

## Note Content

***Only reading the notes is sufficient for understanding the content of this chapter***

### Part 1: Introducrion (Lectures 1-2)

| Files | Topic | Content |
|-------|-------|---------|
|[Lecture01.pdf](Slides/Lecture01.pdf)| Introduction Ⅰ| Syllabus & Concept of Data Science|
|[Lecture02.pdf](Slides/Lecture02.pdf)| Introduction Ⅱ| Four Parts of Data Science |
> This part of content does not summarized as notes.   

### Part 2: Probability (Lectures 3-8)

| Files | Topic | Content |
|-------|-------|---------|
|[Lecture03.pdf](Slides/Lecture03.pdf)| Probability Theory Ⅰ| Probablity Concepts & Functions|
|[Lecture04.pdf](Slides/Lecture04.pdf)| Probability Theory Ⅱ| Independence, Distributions, Mean & Variance |
|[Lecture05.pdf](Slides/Lecture05.pdf)| Random Variable | PMF, Expectation, CDF, Variance |
|[Lecture06.pdf](Slides/Lecture06.pdf)| Common Distributions | Bernoulli, Binomial, Geometric Distribution |
|[Lecture07.pdf](Slides/Lecture07.pdf)| Continuous Random Variable | Uniform, Normal Distribution |
|[Lecture08.pdf](Slides/Lecture08.pdf)| Correlation and Conditional Probability | Expectation of g(x), Correlation, Conditional Probability |

**Probability Note**: [Probability.md](Notes/Probability.md)

### Part 3: Statistics (Lectures 9-12)

| Files | Topic | Content |
|-------|-------|---------|
|[Lecture09.pdf](Slides/Lecture09.pdf)| Maximum Likelihood Estimate | Statistics Definition, Likelihood Function |
|[Lecture10.pdf](Slides/Lecture10.pdf)| Regression Analysis | MLE, Linear Regression |
|[Lecture11.pdf](Slides/Lecture11.pdf)| Confidence Interval Ⅰ| CLT, Confidence Interval |
|[Lecture12.pdf](Slides/Lecture12.pdf)| Confidence Interval Ⅱ| Confidence Interval for Mean |

**Statistics Note**: [Statistics.md](Notes/Statistics.md)

### Part 4: Optimization (Lecture 14-20)

| Files | Topic | Content |
|-------|-------|---------|
|[Lecture14&15.pdf](Slides/Lecture14&15.pdf)| Optimization Formulation | Optimization Definition, Convex Set |
|[Lecture16.pdf](Slides/Lecture16.pdf)| Convex Function Ⅰ| Definition & Proof of Convex Function |
|[Lecture17.pdf](Slides/Lecture17.pdf)| Convex Function Ⅱ| Affine transformation, Pointwise maximum |
|[Lecture18.pdf](Slides/Lecture18.pdf)| Convex Optimization | Examples: LR, LP, Newsvendor |
|[Lecture19.pdf](Slides/Lecture19.pdf)| Practice Questions Ⅰ| Practice |
|[Lecture20.pdf](Slides/Lecture20.pdf)| Practice Questions Ⅱ| Practice |

**Optimization Note**: [Optimization.md](Notes/Optimization.md)

### Part 5: Machine Learning (Lecture 21-24)

| Files | Topic | Content |
|-------|-------|---------|
|[Lecture21.pdf](Slides/Lecture21.pdf)| Machine Learning Intro | Introduction, KNN |
|[Lecture22.pdf](Slides/Lecture22.pdf)| Logistic Regression | Gardient Descent |
|[Lecture23.pdf](Slides/Lecture23.pdf)| Clustering | Unsupervised Learning, K-Means |
|[Lecture24.pdf](Slides/Lecture24.pdf)| Model Selection | K-fold Validation |

**Machine Learning Note**: [Machine Learning.md](Notes/Machine Learning.md)

### Part 6: Review 

| Files | Topic | Content |
|-------|-------|---------|
|[Lecture13.pdf](Slides/Lecture13.pdf)| Midterm Review | Midterm Review |
|[Lecture25.pdf](Slides/Lecture25.pdf)| Final review | Final review |

## Reference Book
- Processing Networks: Fluid Models and Stability , J. G. DaiJ. Michael Harrison, Cambridge University Press, November2020
- Experiments: Planning, Analysis, and Optimization, 3rd Edition, C. F. Jeff Wu & Michael S. Hamada, February 2021
- Revenue Management and Pricing Analytics, Guillermo Gallego & Huseyin Topaloglu, 2019
- Applied Statistics and Probability for Engineers, 3rd Edition, Douglas C. Montgomery & George C. Runger
- Probability and Statistical Inference, 9th edition, Hogg, McKean & Craig
